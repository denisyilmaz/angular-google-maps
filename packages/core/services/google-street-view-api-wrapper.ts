...
export class GoogleStreetViewAPIWrapper {
  ...
  constructor(private _loader: MapsAPILoader, private _zone: NgZone) {
    this._view = new Promise<mapTypes.GoogleStreetViewPanorama>((resolve: () => void) => {
      this._viewResolver = resolve;
    });
  }
  ...
  addCloseClickHandler(handler: Handler): Promise<void> {
    return this._view.then(
        (view: mapTypes.GoogleStreetViewPanorama) => view.addCloseClickHandler(handler));
  }

  addLinksChangeHandler(handler: Handler): Promise<void> {
    return this._view.then(
        (view: mapTypes.GoogleStreetViewPanorama) => view.addLinksChangeHandler(handler));
  }

  addPanoChangeHandler(handler: Handler): Promise<void> {
    return this._view.then(
        (view: mapTypes.GoogleStreetViewPanorama) => view.addPanoChangeHandler(handler));
  }

  addPositionChangeHandler(handler: Handler): Promise<void> {
    return this._view.then(
        (view: mapTypes.GoogleStreetViewPanorama) => view.addPositionChangeHandler(handler));
  }

  addPovChangeHandler(handler: Handler): Promise<void> {
    return this._view.then(
        (view: mapTypes.GoogleStreetViewPanorama) => view.addPovChangeHandler(handler));
  }

  addVisibleChangeHandler(handler: Handler): Promise<void> {
    return this._view.then(
        (view: mapTypes.GoogleStreetViewPanorama) => view.addVisibleChangeHandler(handler));
  }
